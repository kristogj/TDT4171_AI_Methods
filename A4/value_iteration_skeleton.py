...
def value_iteration() -> Any:
    ...
    """
    U_mark = np.zeros(constants.number_states)
    while True:
        U = U_mark.copy()
        delta = 0
        ...
        for state in range(constants.number_states):
            # Calculate all utilities for all possible actions
            utilities = [np.sum([get_transition_probability(state, action, next_state) * U[next_state]
                                 for next_state in get_outcome_states(state, action)])
                         for action in moves.keys()]
            U_mark[state] = get_reward(state) + constants.gamma * max(utilities)

            # Update epsilon
            diff = U_mark[state] - U[state]
            if diff > delta:
                delta = abs(diff)
        # Check if done
...
def extract_policy(value_table: Any) -> Any:
    ...
    """
    policy = np.zeros(constants.number_states)  # Register best action for each state
    for state in range(constants.number_states):
        # Calculate utility for all moves and select the action that gives the maximum value
        utilities = [
            (moves[action], np.sum([get_transition_probability(state, action, next_state) * value_table[next_state]
                                    for next_state in get_outcome_states(state, action)]))
            for action in moves.keys()]
        policy[state] = max(utilities, key=lambda x: x[1])[0]
    return policy
...
def main() -> None:
    ...
    print(value_table, optimal_policy)
